  ...
  next({
    ...
    iterateArticles: function() {
      var nf = this
        ...
        , articleFiles = articleFiles.map(function(file) { return path.join(self.sitePath, file) });
        
      var b = batchfile(articleFiles).transform(function(i, file, data, write) {
        var slug = path.basename(file, '.md')
          , md = marked(data.toString())
          , articleData = articles[articleKeys[i]]
          , urlData = _.extend({slug: slug}, dt["eval"](new Date(articleData.createdAt), 'date-'))
        ...
        var html = self.articleTemplates['_article.html'].template({
          body: md,
          potter: potterData
        });
    ...
    generateIndex: function() {
      ...
      self.potterTemplates.main = html;
      html = self.articleTemplates['layout.html'].template(templateVals);
      fs.writeFile(path.join(self.buildDir, potterData.articles.indexUrl), html, this.next);
    },
    done: function() {
      callback(null);
    }
...
Site.prototype.serve = function() {
  ...
  server.listen(port, function() {
    console.log("Serving up " + (path.join(process.cwd(), _this.buildDir)) + " on port 2222...");
  });
};
