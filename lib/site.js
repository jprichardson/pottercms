...
(function() {
  var DEFAULT_ROCK, Handlebars, P, S, Site, batchfile, bd, buffet, dt, fs, hl, loadFilesInDir, marked, next, packageObj, parent, path, potterPackage, rock, util, _,
  ...
  Site = (function() {
    ...
    function Site(sitePath, rockTemplate) {
      ...
      this.buildDir = path.join(this.sitePath, 'build');
    ...
    Site.prototype.initialize = function(callback) {
      ...
      return next(flow = {
        ...
        loadPotterTemplateFiles: function(err, res) {
          ...
          return loadFilesInDir(P('resources/templates'), modifier, function(err, res) {
            var file, val;
            if (err) {
              flow.error(err);
            } else {
    ...
    Site.prototype.saveData = function(callback) {
      ...
      return next(flow = {
        ...
        articleFile: function() {
          ...
          return fs.writeFile(obj.path, JSON.stringify(obj.data, null, 2), this.next);
        },
        tagFile: function() {
          var obj;
    ...
    Site.prototype.serve = function() {
      ...
      return server.listen(port, function() {
        return console.log("Serving up " + (path.join(process.cwd(), 'build')) + " on port 2222...");
      });
    };
